const PAN_INDIA_LOCALITY_FALLBACKS = [
  {
    label: 'Meridian Business Center (MBC), Lajamni Chowk, Mota Varachha, Surat, Gujarat 394101, India',
    flatHouse: 'Meridian Business Center (MBC)',
    area: 'Lajamni Chowk, Mota Varachha',
    city: 'Surat',
    state: 'Gujarat',
    pinCode: '394101'
  },
...
export function normalizeLocalitySuggestions(suggestions = [], query = '') {
  ...
  const apiSuggestions = suggestions.filter((suggestion) => {
    ...
    if (!key || apiKeys.has(key)) return false
    apiKeys.add(key)
    return true
  })
  ...
  const allSuggestions = [...apiSuggestions, ...fallbackSuggestions].filter((suggestion) => {
    const key = `${(suggestion?.label || '')}|${(suggestion?.area || '')}|${(suggestion?.city || '')}`.toLowerCase()
    if (!key) return false
    if (allSuggestionKeys.has(key)) return false
    allSuggestionKeys.add(key)
    return true
  })
