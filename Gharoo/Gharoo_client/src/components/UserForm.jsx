...
function getMapMatchedArea(suggestion) {
  return coalesce(
    ...
    suggestion?.street,
    suggestion?.road,
    suggestion?.raw?.properties?.street,
    suggestion?.raw?.properties?.road,
    suggestion?.area
  )
}
...
export default function UserForm() {
  ...
  async function applyResolvedSuggestion(suggestion, latitude, longitude, options = {}) {
    ...
    setFormData((prev) => {
      ...
      const nextArea = preferExactLocation
        ? coalesce(suggestion?.area, getMapMatchedArea(suggestion))
        : coalesce(suggestion?.area, prev.area)
      ...
      // Reverse geocoding can sometimes resolve a premise/house number. Keep a
      // manually entered value if the provider cannot, rather than erasing it.
  ...
  async function handleCurrentLocation() {
    ...
    try {
      ...
      if (address) {
        ...
        alert('We found your location, but could not identify its address. Please complete the address fields manually or move the map pin.')
      }
        ...
        <form onSubmit={handleSubmit} className="booking-form">
          ...
          <div className="booking-fields">
            ...
            <DraggableAddressMap
              ...
              idleText="Use current location to detect your full service address."
