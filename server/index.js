...
// Google Places/Geocoding require billing. They stay disabled unless a
// production environment explicitly opts in; the no-billing fallbacks below
// continue to provide locality and reverse-address results in development.
const ENABLE_GOOGLE_GEOCODER = process.env.ENABLE_GOOGLE_GEOCODER === 'true';
...
// Converts Google's component list into the fields used by the booking form.
// A coordinate cannot always identify an apartment/flat, so that field is
// intentionally best-effort and remains editable in the browser.
function normalizeGoogleGeocodeResult(result, latitude, longitude) {
  ...
  return {
    ...
    // Do not invent a flat number. Google may return a premise, subpremise,
    // or a street address; use only what it resolved for this coordinate.
    flatHouse: [subpremise, premise, streetNumber, route].filter(Boolean).join(', '),
    street: route,
    area,
    city,
...
async function fetchGoogleReverseAddress(latitude, longitude) {
  ...
  if (payload.status !== 'OK' || !Array.isArray(payload.results) || !payload.results.length) {
    throw new Error(`Google Geocoding API: ${payload.status || 'No results'}${payload.error_message ? ` - ${payload.error_message}` : ''}`);
  }
  return normalizeGoogleGeocodeResult(payload.results[0], latitude, longitude);
}

function normalizeGeoapifyReverseAddress(feature, latitude, longitude) {
  const properties = feature?.properties || {};
  const city = properties.city || properties.town || properties.village || properties.municipality || '';
  const area = properties.suburb || properties.neighbourhood || properties.neighborhood ||
    properties.city_district || properties.district || city;
  const street = properties.street || properties.road || '';

  return {
    id: `geoapify-${properties.place_id || `${latitude},${longitude}`}`,
    source: 'geoapify',
    ...
    flatHouse: [properties.name, properties.housenumber, street].filter(Boolean).join(', '),
    street,
    area,
    city,
...
async function fetchGeoapifyReverseAddress(latitude, longitude) {
  ...
  return feature ? normalizeGeoapifyReverseAddress(feature, latitude, longitude) : null;
}

// Free fallback for address auto-fill. Nominatim is backed by OpenStreetMap and
// needs no API key. Keeping it server-side avoids browser CORS restrictions.
function normalizeNominatimReverseAddress(result, latitude, longitude) {
  const address = result?.address || {};
  const city = address.city || address.town || address.village || address.municipality ||
    address.city_district || address.county || '';
  const area = address.suburb || address.neighbourhood || address.neighborhood ||
    address.quarter || address.city_district || address.residential || address.hamlet || city;
  ...
  return {
    ...
    flatHouse: [house, street].filter(Boolean).join(', '),
    street,
    area,
    city,
...
async function fetchNominatimReverseAddress(latitude, longitude) {
  const params = new URLSearchParams({
    lat: String(latitude),
    lon: String(longitude),
    format: 'jsonv2',
  ...
  const response = await fetch(`https://nominatim.openstreetmap.org/reverse?${params}`, {
    headers: {
      // Nominatim requires identifying applications. This is not a credential.
      'User-Agent': 'GharooCare/1.0 (address auto-fill)'
    }
  });
  if (!response.ok) {
  ...
  return result?.display_name ? normalizeNominatimReverseAddress(result, latitude, longitude) : null;
}

// A second free source improves city-level data in India. OpenStreetMap may
// return a taluka (an administrative district) as the city for a GPS pin;
// BigDataCloud correctly identifies the enclosing city without an API key.
async function fetchCityForCoordinates(latitude, longitude) {
  const params = new URLSearchParams({
...
function isAdministrativeArea(value = '') {
  return /\b(taluka|tehsil|district|division|county)\b/i.test(value);
}
...
async function fetchGoogleLocalitySuggestions(query) {
  ...
  if (text.length < 3 || !ENABLE_GOOGLE_GEOCODER || !GOOGLE_PLACES_API_KEY) return [];
...
// The browser sends only GPS coordinates obtained after the user grants
// permission. Google is used when configured; otherwise free OpenStreetMap
// data keeps address auto-fill working without an API key.
app.get('/api/reverse-geocode', async (req, res) => {
  ...
  try {
    let address = null;
    if (ENABLE_GOOGLE_GEOCODER && GOOGLE_PLACES_API_KEY) {
      try {
        address = await fetchGoogleReverseAddress(latitude, longitude);
      } catch (googleError) {
        console.warn('Google reverse geocoding failed, using Geoapify fallback:', googleError.message);
      }
    }
    if (!address && GEOAPIFY_API_KEY) {
      address = await fetchGeoapifyReverseAddress(latitude, longitude);
    }
    // Nominatim remains the final fallback even when a configured commercial
    // provider rejects a request (for example, an API key restriction or a
    // temporarily disabled Google Geocoding API). A failed provider must not
    // prevent the booking form from filling a valid address.
    if (!address) {
      address = await fetchNominatimReverseAddress(latitude, longitude);
    }
    if (!address) {
      return res.status(404).json({ success: false, message: 'No address was found for this location.' });
    }

    // Do not let a taluka overwrite the actual city. This supplementary lookup
    // is best-effort, so address filling still works if it is unavailable.
    if (!ENABLE_GOOGLE_GEOCODER || address.source !== 'google') {
      try {
        const cityData = await fetchCityForCoordinates(latitude, longitude);
        if (cityData?.city) address.city = cityData.city;
        if (!address.state && cityData?.state) address.state = cityData.state;
        if (isAdministrativeArea(address.area)) {
          address.area = cityData?.area && !isAdministrativeArea(cityData.area)
            ? cityData.area
            : '';
        }
      } catch (cityError) {
        console.warn('City fallback failed:', cityError.message);
      }
    }
    return res.json({ success: true, address });
  } catch (error) {
    console.error('Reverse geocoding failed:', error.message);
    return res.status(502).json({ success: false, message: error.message || 'Google address lookup is temporarily unavailable.' });
  }
});
