...
export default function UserPanelServices() {
  const [servicesData, setServicesData] = useState({ header: { label: '', title: '', description: '' }, items: [] });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedSettings = { ...getResData.data, services: servicesData };
  ...
  const handleHeaderChange = (field, value) => {
    setServicesData({ ...servicesData, header: { ...servicesData.header, [field]: value } });
  };
  ...
  const removeService = (index) => {
    const newItems = servicesData.items.filter((_, i) => i !== index);
    setServicesData({ ...servicesData, items: newItems });
  };
    ...
    <div className="container-fluid px-3 px-lg-4 py-4">
      <div className="page-heading mb-4">
        <div className="page-heading-copy">
          <span className="page-icon"><i className="bi bi-briefcase" aria-hidden="true"></i></span>
          <div>
            <p className="eyebrow mb-1">User Panel</p>
            <h1 className="h3 mb-1">Services Section</h1>
          </div>
        </div>
        ...
        <div className="panel-body">
          <div className="row g-3">
            <div className="col-md-4">
              ...
              <input type="text" className="form-control" value={servicesData.header.label} onChange={(e) => handleHeaderChange('label', e.target.value)} />
            </div>
            <div className="col-md-8">
              <label className="form-label">Title</label>
              <input type="text" className="form-control" value={servicesData.header.title} onChange={(e) => handleHeaderChange('title', e.target.value)} />
            </div>
            <div className="col-md-12">
              <label className="form-label">Description</label>
              <textarea className="form-control" rows={2} value={servicesData.header.description} onChange={(e) => handleHeaderChange('description', e.target.value)} />
            </div>
          </div>
        </div>
      </section>
          ...
          {servicesData.items.map((service, index) => (
            <div key={index} className="mb-4 p-3 border rounded">
              <div className="d-flex justify-content-between mb-2">
                ...
                <button className="btn btn-sm btn-danger" onClick={() => removeService(index)}><i className="bi bi-trash" /></button>
              </div>
              <div className="row g-3">
